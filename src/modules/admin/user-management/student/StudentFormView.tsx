...
import { AddStudentPayloadType, GetFacultiesType, UpdateStudentPayloadType } from '@/api/admin/types';
...
export default function StudentFormView() {
  ...
  const item: AddStudentPayloadType = id
    ? { ...passedData }
    : {
      first_name: "",
      last_name: "",
  ...
  const { mutate: addStudent } =
    api.admin.studentUsers.addStudent.useMutation({
      ...
      onSuccess: () => {
        ...
        navigate("/admin/user-management/students");
      },
      onError: (error) => {
  ...
  const onSubmit = async (item: z.infer<typeof formSchema>) => {
    try {
      ...
      if (id) {
        ...
        // Call update API
        await updateStudent(formData as unknown as UpdateStudentPayloadType);
      } else {
        // For add form
        formData.append("createby", (item.createby || 1).toString());

        // Call add API
        await addStudent(formData as unknown as AddStudentPayloadType);
      }
    } catch (error) {
    ...
    <section className="m-4">
      ...
      <div className="p-6 bg-white rounded-lg">
        <div className='flex mb-8'>
          <div className='me-5'>
            <Link to={'/admin/user-management/students'}>
